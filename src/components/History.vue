...
<script>
	...
	export default {
		...
		mounted () {
			...
			//var overviewSubTitle = this.$echarts.format.formatTime('yyyy年MM月dd', new Date());

			//图表定位
			var chartGridTop = 40;
			var chartGridHeight = 80;
			...
			function makeGrid(top, height, opt) {
				return this.$echarts.util.merge({
					left: 90,
					right: 90,
					top: top,
					height: height
			...
			var makeGridInChart = makeGrid.bind(this);
			...
			var makeXAxisInChart = makeXAxis.bind(this);
			...
			var makeYAxisInChart = makeYAxis.bind(this);
			...
			var makeGridDataInChart = makeGridData.bind(this);
			...
			var option = {
				...
				//直角坐标系内绘图网格
				grid: [
					makeGridInChart(chartGridTop, chartGridHeight),
					makeGridInChart(chartGridTop + (chartGridHeight + 25), chartGridHeight),
					makeGridInChart(chartGridTop + (chartGridHeight + 25) * 2, chartGridHeight),
					makeGridInChart(chartGridTop + (chartGridHeight + 25) * 3, chartGridHeight),
					makeGridInChart(chartGridTop + (chartGridHeight + 25) * 4, chartGridHeight),
				],
				xAxis: [
					makeXAxisInChart(0),
					makeXAxisInChart(1),
				...
				series: [
					makeGridDataInChart(0, 0, 'line', 'PM2.5', orddbhost1),

					makeGridDataInChart(1, 1, 'line', '甲醛', orddbtime1, {
						stack: 'DBTIME'
					}),

					makeGridDataInChart(2, 2, 'line', '光照', orddbproc1, {}),
					makeGridDataInChart(3, 3, 'line', '湿度', orddbstor1),
					makeGridDataInChart(4, 4, 'line', '温度', orddbstor2),
				]
			};
